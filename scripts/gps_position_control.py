...
class Global_position_control(Node):
    ...
    def timer_callback(self):
        ...
        #TODO : set_start_position이 실행되기 전에 current_local이
        # 채워져 있어야 하는데 _cb가 어떤 타이밍에 실행되는지 몰라서
        #이 타이밍에 current_local이 채워져 있는지 알 수 없음
        # counter가 증가하지 않음
        if self.current_global is None or self.current_local is None:
            if not self.wait_log_printed:
                print("GPS 및 Local 위치 수신 대기 중...")
