...
#공격할 때 드론 끼리 부딧힘. 직선이 아닌 경로로 다양한 방향에서 타격하거나,
#목표가 좀 커서 다른 목표 좌표를 잡고가면 안 부딧힐듯
import rclpy
import math
import threading
...
class DroneController(Node):
    ...
    def send_trajectory(self):
        ...
        if self.mission_phase == 0:
            # make_formation.py와 동일한 순차 이륙/대형 진입 로직 적용
            #CHANGE
            HOVER_ALTITUDE = -1.0         # 초기 이륙 고도 (m)
            HOVER_WAIT_TIME = 30       # 모든 기체가 띄워진 뒤 대기 시간(0.1s tick 기준 5초)
            SEQUENCE_INTERVAL = 5       # 드론별 출발 간격(0.1s tick 기준 1초)
            ...
            # offboard/arm 명령(약 30 tick)을 감안해 출발 기준점을 보정
            flight_time = self.counter - (self.warmup_steps + 30)  # 명령 송신 시간 동안 흐른 tick을 빼서 make_formation.py와 동일하게 맞춤
            my_start_time = HOVER_WAIT_TIME + (self.drone_id - 2) * SEQUENCE_INTERVAL

            if flight_time < my_start_time:
                # 순서가 올 때까지 스폰 위치 위에서 지정 고도로 대기
